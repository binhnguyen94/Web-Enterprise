...
while ($kq=mysql_fetch_array($qr)) {
	...
	echo "<tr>";
		...
		echo "<td>".$kq['email']."</td>";
		echo "<td>[<a href='?act=sinhvien&mod=sua&id=$kq[masinhvien]'>Sửa</a>] | [<a href='?act=sinhvien&mod=xoa&id=$kq[masinhvien]' onclick='return checkDel()'>Xóa</a>]</td>";
	echo "</tr>";
}
?>
</table>
<?php 
	if($_GET['id_lophoc']){
		pageDivider("select count(*) from tbl_sinhvien where id_lophoc=$_GET[id_lophoc]"); 
	}
	elseif($_GET['name']){
		pageDivider("select count(*) from tbl_sinhvien where hoten like'%$_GET[name]%'"); 
	}
	else{
		pageDivider("select count(*) from tbl_sinhvien"); 
	}
?>